...
class StackingFramework:
    ...
    def train_base_models(self, X_train, y_train, cv, scoring):
        ...
        for name, (model, hyperparameters) in self.base_classifiers.items():
            ...
            # Provide default hyperparameters for the base model if not specified by the user
            if not hyperparameters:
                default_hyperparameters = self.get_default_base_hyperparameters(name)
                all_hyperparameters.update({f'model__{param}': value for param, value in default_hyperparameters.items()})
